...
class CobrancaFalhouDtoUnitTests {
	...
	void shouldConstructFromJson() throws JsonProcessingException {
		...
		CobrancaFalhouDto orderPaid = mapper.readValue(json, CobrancaFalhouDto.class);
	...
	void shouldViewToStringWithOrderId() {
		var orderPaid = CobrancaFalhouDto.builder().orderId(ORDER_ID).build();
		assertThat(orderPaid).isNotNull()
