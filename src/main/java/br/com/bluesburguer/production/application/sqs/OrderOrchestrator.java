...
public class OrderOrchestrator {
	...
	// Pedido
	@SqsListener(value = "${queue.order.registered:order-registered.fifo}", deletionPolicy = SqsMessageDeletionPolicy.NEVER)
	...
	@SqsListener(value = "${queue.order.confirmed:order-confirmed.fifo}", deletionPolicy = SqsMessageDeletionPolicy.NEVER)
    ...
    public void handle(PedidoCanceladoDto order, Acknowledgment ack) throws JsonProcessingException {
		log.info("Event received on queue order-canceled: {}", order);
    	if (ObjectUtils.isNotEmpty(order) && update(order, Step.ORDER, Fase.CANCELED)) {
    		ack.acknowledge();
    	}
    }
	...
	@SqsListener(value = "${queue.order.failed-on-payment:order-failed-on-payment.fifo}", deletionPolicy = SqsMessageDeletionPolicy.NEVER)
	...
	// Entrega
	@SqsListener(value = "${queue.order.scheduled:order-scheduled.fifo}", deletionPolicy = SqsMessageDeletionPolicy.NEVER)
	...
	@SqsListener(value = "${queue.order.failed-delivery:order-failed-delivery.fifo}", deletionPolicy = SqsMessageDeletionPolicy.NEVER)
	...
	@SqsListener(value = "${queue.order.performed-delivery:order-performed-delivery.fifo}", deletionPolicy = SqsMessageDeletionPolicy.NEVER)
	...
	// NotaFiscal
	@SqsListener(value = "${queue.order.invoice-issued:order-invoice-issued.fifo}", deletionPolicy = SqsMessageDeletionPolicy.NEVER)
	...
	@SqsListener(value = "${queue.order.invoice-failed-issued:order-invoice-failed-issued.fifo}", deletionPolicy = SqsMessageDeletionPolicy.NEVER)
